-- Bastion Lunchbox Fixes v1.0.5 compatibility prefix
--
...
-- behavior by conditionally starting the independent Codex modules that
-- Bridge v1 starts, plus the separately packaged vehicle aggro diagnostic.
-- This prefix makes no gameplay changes of its own.
--
...
do
    if not rawget(_G, 'CodexModuleBridge') then
        local bridge = {
            revision = 'bastion-compat-v1.0.5',
            modules = {},
            status = 'starting'
        }
        ...
        local function bridge_report(name, status)
            ...
            pcall(function()
                ...
                file:write('Codex Module Bridge compatibility via Bastion Lunchbox Fix v1.0.5\n')
